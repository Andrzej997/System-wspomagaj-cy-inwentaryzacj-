...
public class ValidationErrorMessanger {
    ...
    public static void showErrorMessage(JComponent comp, String text) {
        if (ValidationErrorMessanger.errBalloon != null) {
            ValidationErrorMessanger.errBalloon.closeBalloon();
        }
        BalloonTipStyle style = new MinimalBalloonStyle(new Color(246, 197, 192, 220), 5);
        ValidationErrorMessanger.errBalloon = new BalloonTip(comp, new JLabel("<html><font color=\"#6f150d\">" + text + "</font></html>"),
                style, BalloonTip.Orientation.LEFT_ABOVE, BalloonTip.AttachLocation.ALIGNED, 15, 10, false);
        ValidationErrorMessanger.errBalloon.addDefaultMouseListener(false);
        TimingUtils.showTimedBalloon(ValidationErrorMessanger.errBalloon, 3000);
    }

    /**
     * Retrieve an instance of the balloon tip style to be used throughout the
     * application
     *
    ...
    /**
     * Retrieve an instance of the balloon tip positioner to be used throughout
     * the application
     *
    ...
    /**
     * Wraps a string in a monospace font tag (for use in a HTML-formatted
     * label)
     *
